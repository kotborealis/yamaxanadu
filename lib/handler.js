...
module.exports = (schema, __schemaType) => ({
	get(target, prop, reciever){
		...
		if(!(prop in schema)){
			throw new ReferenceError(`Trying to get unknown prop ${prop.toString().slice()}`, schema);
		}
	...
	set(target, prop, value){
		if(!(prop in schema)){
			throw new ReferenceError(`Trying to set unknown prop ${prop.toString().slice()}`, schema);
		}
		...
		if(!valid){
			throw new TypeError(`Expected property ${prop.toString().slice()} to be of type ${
